...
rttp=[]
for i in [x for x in os.listdir(path+'Output/Archive/') if x.startswith('rttp')]:
    rttp.append(pd.read_csv(path+'Output/Archive/'+i))
...
sctp=[]
for i in [x for x in os.listdir(path+'Output/Archive/') if x.startswith('sctp')]:
    sctp.append(pd.read_csv(path+'Output/Archive/'+i))
